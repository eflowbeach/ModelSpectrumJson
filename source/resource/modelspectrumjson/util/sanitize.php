...
function sanitize_strict_string($string, $min, $max) {
...
//Takes a full URL with parameters and encodes the parameters for safe use
//as output to a form
function EncodeURL($url)
{
    ...
    else 
    {
        $encodedParamStr = "";
        $startPos = strpos($url, '?');
        $serverPath = substr($url, 0, $startPos + 1);
        $paramsStr = substr($url, $startPos + 1);
        $params = explode('&', $paramsStr);
        foreach($params as $param)
        {
            $paramNameValue = explode('=', $param);

            $paramNameValue[1] = urlencode($paramNameValue[1]);

            $encodedParamStr .= implode('&amp', $paramNameValue);
        }
        $finalURL = $serverPath . $encodedParamStr;
        return $finalURL;
    }
}
